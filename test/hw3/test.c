...
void printCharMatrixWithRows(char **matrix, int rows)
{
    for (int i = 0; i < rows; i++)
    {
        printf("%s", matrix[i]);
...
void printCharArr(char *matrix, int len)
{
    for (int i = 0; i < len; i++)
    {
        printf("%c", matrix[i]);
...
int wordCoveredForAllTiles(GameState *game, char **wordMatrix, int wordHeight, int row, int col, char direction, const char *tiles, int tilesLength)
{
    ...
    // strcpy(copyOfTiles, tiles);
    // copyOfTiles[tilesLength] = '\0';
    int updatedRow = row;
    int updatedCol = col;
    ...
    *num_tiles_placed = numCharInTiles(tiles);
    ...
    if (invalidity == 0 && initialEmptyBoard == 1)
    {
        ...
        if (invalidity == 0)
        {
            ...
            if (WORDM == NULL || wordMatrixCreated == 0)
            {
                WORDM = createWordsMatrix("tests/words.txt", &wordH, &wordLength);
                wordMatrixCreated = 1;
            }
            // printCharMatrix(wordMatrix, wordHeight, wordLength);
            // handle case if the tiles have spaces
            // char x = wordMatrix[0][0];
            // (void)x;
            // printCharMatrix(wordMatrix, wordHeight, wordLength);
            // handle case if the tiles have spaces
            int wordsCovered = wordCoveredForAllTiles(game, WORDM, wordH, row, col, direction, tiles, tilesLength);
        ...
        if (invalidity == 0)
        {
            ...
            int validWord = boardIsValid(newGame, WORDM, wordH, row, col, direction, tiles, tilesLength);
            if (validWord == 1)
            {
                invalidity = 1;
            }
    ...
    if (invalidity == 1)
    {
        // free everything that was dynamically allocated in this function
        // if (freedWordMatrixFlag == 0 && wordMatrix != NULL)
        // {
        //     freeCharMatrix(wordMatrix, wordHeight);
        // }
        *num_tiles_placed = 0;
...
void free_game_state(GameState *game)
{
    ...
    placedTile = 0;
    if (wordMatrixCreated == 1 || WORDM != NULL)
    {
        freeCharMatrix(WORDM, wordH);
        WORDM = NULL;
        wordMatrixCreated = 0;
    }
    (void)game;
