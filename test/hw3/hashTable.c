...
#define CAPACITY 234885 // Size of the HashTable.
...
unsigned long hash_function(const char *str) {
    ...
    while ((c = *str++)) {
        hash = (hash * 31) + c; // A simple hash function
    }

    return hash % CAPACITY; // Ensure the hash value fits within the range of valid indices
}
...
void insertWordsFromFileIntoHashtable(const char* filename, HashTable* ht) {
...
int main() {
    HashTable* ht = create_table(CAPACITY); // Initialize your hashtable

    insertWordsFromFileIntoHashtable("/Users/gracewang/Documents/Stony Brook/CSE220/HW/test/words.txt", ht);
    // print_table(ht);
    printf("%d NUM OF COLLISONS", count);
    // Perform any operations with your populated hashtable
