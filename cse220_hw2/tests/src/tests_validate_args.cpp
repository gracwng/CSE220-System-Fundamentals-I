...
TEST_F(validate_args_TestSuite, acceptable_args) {
	int status = run_using_system("-o ./tests/actual_outputs/.sbu -i ./tests/images/seawolf.sbu");
...
TEST_F(validate_args_TestSuite, missing_i_arg) {
	int status = run_using_system("-o ./tests/actual_output/.ppm -r \"I love SBU\",\"fonts/font3.txt\",1,4,2");
	EXPECT_EQ(MISSING_ARGUMENT, WEXITSTATUS(status));
}
